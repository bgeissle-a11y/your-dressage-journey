...
import { getTestData, getShortLabel, isFullDataAvailable, getAssessItems } from '../../services/testDatabase';
...
export default function ShowPrepForm() {
  ...
  // Flag state: { [testId]: { [itemId]: boolean } }
  const [flagState, setFlagState] = useState({});
  // Active test in sidebar
  const [activeTestId, setActiveTestId] = useState(null);
  // Mobile sidebar toggle
  const [mobileSidebarOpen, setMobileSidebarOpen] = useState(false);

  // Debounce timer for flag persistence
  const flagSaveTimerRef = useRef(null);
  ...
  // Derive selected tests from form data
  ...
  const selectedTests = [];
  for (let i = 1; i <= maxTestSlots; i++) {
    const val = formData[`test${i}`];
    if (val) {
      const opt = testOptions.find(t => t.value === val);
      selectedTests.push({ value: val, label: opt?.label || val });
    }
  }

  // Auto-switch sidebar to first selected test, or clear if none
  useEffect(() => {
    if (selectedTests.length === 0) {
      setActiveTestId(null);
    } else if (!activeTestId || !selectedTests.find(t => t.value === activeTestId)) {
      setActiveTestId(selectedTests[0].value);
    }
  }, [selectedTests.map(t => t.value).join(',')]);

  // Total flag count across all tests
  ...
  async function loadExisting() {
    ...
    if (result.success) {
      const d = result.data;
      const tests = d.testsSelected || (d.tests?.selected) || [];
      setFormData({
        showName: d.showName || d.showDetails?.name || '',
        showDuration: d.showDuration || d.showDetails?.duration || 'single',
        showDateStart: d.showDateStart || d.showDetails?.dateStart || '',
        showDateEnd: d.showDateEnd || d.showDetails?.dateEnd || '',
        showType: d.showType || d.showDetails?.type || '',
        showTypeOther: d.showTypeOther || d.showDetails?.typeOther || '',
        showLocation: d.showLocation || d.showDetails?.location || '',
        horseName: d.horseName || d.horse?.name || '',
        currentLevel: d.currentLevel || d.horse?.currentLevel || '',
        showExperience: d.showExperience || d.horse?.showExperience || '',
        testType: d.testType || d.tests?.type || 'standard',
        ...
        concern1: (d.concerns?.additionalConcerns?.[0]) || (d.concerns && d.concerns[0]) || '',
        concern2: (d.concerns?.additionalConcerns?.[1]) || (d.concerns && d.concerns[1]) || '',
        concern3: (d.concerns?.additionalConcerns?.[2]) || (d.concerns && d.concerns[2]) || '',
        ridingFrequency: d.ridingFrequency || d.resources?.ridingFrequency || '',
        coachAccess: d.coachAccess || d.resources?.coachAccess || '',
        availableResources: d.availableResources || d.resources?.available || [],
        constraints: d.constraints || d.resources?.constraints || '',
        additionalInfo: d.additionalInfo || '',
        status: d.status || 'draft'
      });

      // Restore flag state from flaggedByTest
      if (d.concerns?.flaggedByTest) {
        const restored = {};
        d.concerns.flaggedByTest.forEach(entry => {
  ...
  function buildFlaggedByTest() {
    return selectedTests
      .filter(t => flagState[t.value] && Object.values(flagState[t.value]).some(Boolean))
      .map(t => {
        const data = getTestData(t.value);
        const assessItems = data?.assessItems || [];
        const flagged = assessItems.filter(item => flagState[t.value]?.[item.id]);
        return {
          testId: t.value,
          testLabel: t.label,
          flaggedItems: flagged.map(item => ({
            id: item.id,
            text: item.text,
            coeff: item.coeff
          })),
          doubleCoeffFlags: flagged.filter(i => i.coeff).map(i => i.text)
        };
      });
  }
  ...
  async function handleSubmit(e) {
    ...
    const data = {
      showDetails: {
        ...
        duration: formData.showDuration,
        dateStart: formData.showDateStart,
        dateEnd: formData.showDuration === 'multi' ? formData.showDateEnd : null,
        type: formData.showType,
        typeOther: formData.showType === 'other' ? formData.showTypeOther : null,
        location: formData.showLocation || null
      },
      tests: {
        type: formData.testType,
        selected: collectTestSelections()
      },
      horse: {
        name: formData.horseName,
        currentLevel: formData.currentLevel,
        showExperience: formData.showExperience
      },
      goals: [formData.goal1, formData.goal2, formData.goal3].filter(Boolean),
      concerns: {
        flaggedByTest: buildFlaggedByTest(),
        additionalConcerns: [formData.concern1, formData.concern2, formData.concern3].filter(Boolean)
      },
      resources: {
        ridingFrequency: formData.ridingFrequency,
        coachAccess: formData.coachAccess,
        available: formData.availableResources,
        constraints: formData.constraints || null
      },
      additionalInfo: formData.additionalInfo || null,
      status: formData.status,
      // Keep flat fields for backward compat with list/plan views
  ...
  async function handleSaveDraft() {
    ...
    // Re-use submit data structure but don't navigate
    const data = {
      showDetails: {
        name: formData.showName,
        duration: formData.showDuration,
  ...
  return (
    <div className="form-page">
      ...
      {/* Mobile sidebar (inline above form) */}
          ...
          <form onSubmit={handleSubmit} autoComplete="off">
            <div className="form-card">
              ...
              <FormSection title="Tests You're Riding" description="Select up to 6 tests. Use the toggle to switch between standard and freestyle.">
                ...
                {selectedTests.length > 0 && (
                  <div className="sp-sidebar-hint">
                    Use the Test Reference panel to explore movements, coefficients, and flag priorities for your selected tests.
                  </div>
                )}
              </FormSection>
              ...
              {/* Section 5: Concerns */}
              <FormSection title="Your Concerns" description="Flag specific movements in the sidebar, then add any other concerns below">
                {/* Part 1: Flags callout */}
                <FlagsCallout
                  flagState={flagState}
                  selectedTests={selectedTests}
                />
                ...
                <div className="sp-concerns-label">Other concerns — things that aren't specific test movements</div>
                <div className="show-prep-info-box">
                  <strong>Why we ask:</strong> Naming your concerns helps us address them directly in your preparation plan. Common concerns include nerves, horse behavior, logistics, or recovery from setbacks.
                </div>
                <div className="goal-concern-item">
              ...
              <div className="form-actions">
                ...
                <button
                  type="button"
                  className="btn btn-outline sp-save-draft-btn"
                  onClick={handleSaveDraft}
                  disabled={loading || saveFeedback === 'saving'}
                >
                  {saveFeedback === 'saving' ? 'Saving...' :
                   saveFeedback === 'saved' ? 'Draft Saved' :
...
function FlagsCallout({ flagState, selectedTests }) {
  const allFlags = [];
  const hasMultipleTests = selectedTests.length > 1;

  selectedTests.forEach(test => {
    const testFlags = flagState[test.value] || {};
    const data = getTestData(test.value);
    if (!data) return;

    (data.assessItems || []).forEach(item => {
      if (testFlags[item.id]) {
        allFlags.push({
          ...item,
          testLabel: hasMultipleTests ? (data.shortLabel || test.label) : null
        });
      }
    });
  });

  const hasCoeff = allFlags.some(f => f.coeff);

  if (allFlags.length === 0) {
    return (
      <div className="sp-flags-callout empty">
        <div className="sp-flags-callout-header">
          <span className="sp-flags-icon">&#9873;</span>
          <span>Flagged Movements</span>
          <span className="sp-flags-count">0</span>
        </div>
        <div className="sp-flags-empty-msg">
          No movements flagged yet. Use the <strong>Flag for Prep</strong> tab in the Test Reference panel to flag movements you want to prioritize in your preparation plan.
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="sp-flags-callout">
      <div className="sp-flags-callout-header">
        <span className="sp-flags-icon">&#9873;</span>
        <span>Flagged Movements</span>
        <span className="sp-flags-count">{allFlags.length}</span>
      </div>
      <div className="sp-flags-chips">
        {allFlags.map((flag, i) => (
          <span key={`${flag.id}-${i}`} className={`sp-flag-chip${flag.coeff ? ' coeff' : ''}`}>
            {flag.testLabel && <span className="sp-flag-test-badge">{flag.testLabel}</span>}
            {flag.text}
            {flag.coeff && <span className="sp-flag-x2">x2</span>}
          </span>
        ))}
      </div>
      {hasCoeff && (
        <div className="sp-flags-coeff-note">
          Double-coefficient movements will be prioritized in your Technical preparation output.
        </div>
      )}
    </div>
  );
